  ...
  const onAddOrder = (Dadr, Dsize, Dtime, Dtype, Dtell) => {
    if (Dadr && Dsize && Dtime && Dtell) {
      ...
      let newdata = data
        ? data +
          ';' +
          `{address:'${Dadr}',size:'${Dsize}',time:'${Dtime}',type:'${Dtype}',type:'${Dtell}'}`
        : `{address:'${Dadr}',size:'${Dsize}',time:'${Dtime}',type:'${Dtype}',type:'${Dtell}'}`
      // 存到本地
    ...
    <div>
      ...
      <PaymentItem>
        ...
        <div className="payment-item">
          <div className="payment-item__time">
            {date.getMonth() + 1}-{date.getDate()} (今天){' '}
            {detaildata.PaymentItem}
          </div>
      ...
      <PaymentCoupon>
        <div className="coupon-top">
          ...
          <div className="coupon-top__right iconfont color-gray">
            暂无可用 &#xe61f;
          </div>
        </div>
        <div className="coupon-bottom">
          <div className="coupon-bottom__card">卡</div>
          <div className="coupon-bottom__right iconfont ">
            购卡得福利, 立即购买{' '}
            <span className="iconfont color-gray">&#xe61f;</span>
          </div>
        </div>
      </PaymentCoupon>
      <PaymentAddress>
      ...
      <PaymentFooter>
        ...
        <div className="footer-botton" onClick={handlebackOk}>
          去结算
        </div>
      </PaymentFooter>
      <SuccessPopup
